  ...
  return (
    <article ref={containerRef} className="w-full justify-start">
      <section className="relative inline-block text-left">
        ...
        <section
          className={`w-full absolute mt-2 bg-white border border-gray-300 rounded-md shadow-lg z-10 transform transition-all duration-300
        ...
        >
          {options.map((opt, index) => (
            <section
              key={opt.label}
              className={`px-4 py-2 hover:bg-gray-100 cursor-pointer md:text-lg ${
                index < options.length - 1 ? "border-b border-gray-200 text-gray-800" : ""
              }`}
              onClick={() => {
                opt.onClick();
                setIsOpen(false);
